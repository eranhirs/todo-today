...
If the focused todo still isn't in the rendered list after rAF retries — for instance because it's a completed todo that's been paginated away from the initial load — the focus-tab recovery path fetches the todo by id (`GET /todos/{id}`), then drives the search bar with its text (`setSearchQuery(todo.text)`). The existing search machinery hits the backend `/todos/search` endpoint, which scans every todo regardless of pagination, and returns the row. The recovery then polls for the row to appear in the DOM and scrolls to it. Only if even the search-driven path fails after ~5s does the user see a final "couldn't reach this todo" toast.
