...
8. `red_flags`: semantic red flags you want to raise on specific todos. Each has `todo_id` (the ID of the todo to flag), `label` (a short generic phenomenon name), and `explanation` (1 sentence explaining the concern). The `label` must describe a general anti-pattern or phenomenon — NOT specific details. Think of it like a category name. Good labels: "Over-engineering", "Scope creep", "Silent failure", "Premature abstraction", "Unnecessary complexity", "Unilateral action", "Incomplete implementation", "Missing error handling", "Tight coupling", "Magic values". Bad labels: "Added extra Redis cache layer", "Changed the auth flow without asking". The explanation field is where you describe the specific concern. Only raise flags when you genuinely see a problem in the session activity.
