...
8. `red_flags`: semantic red flags you want to raise on specific todos. Each has `todo_id` (the ID of the todo to flag), `label` (a short 2-5 word label for the flag), and `explanation` (1 sentence explaining the concern). Use this to flag concerns that go beyond simple pattern-matching — things like over-engineering, scope creep, suspicious workarounds, silent error swallowing, premature abstraction, unnecessary complexity, or any code smell you notice in the session activity. Only raise flags when you genuinely see a problem. These flags are shown to the user alongside automated coping-phrase detections but styled differently as AI-raised flags.
