...
3. **User-created todos (source="user") are protected.** Only action allowed: set status to "stale" via status_updates, OR mark as "completed" if run_status is "done" (Claude ran the task). Cannot change text.
...
10. **Promote completed runs (be conservative)**: Todos with status "waiting" and run_status "done" have finished running but haven't been confirmed as complete. Only mark as "completed" if ALL of these are true: (a) session state is "ended", (b) the work was clearly accomplished, AND (c) the conversation has concluded — there is no ongoing discussion, unresolved questions, or user uncertainty in the final messages. **Leave as "waiting" if**: the user's last messages raise concerns, ask questions, express uncertainty, or continue discussing the topic — even if the run itself succeeded. A run finishing successfully does NOT mean the todo is done; the user may still be evaluating the result or discussing follow-ups.
11. **Auto-accept messages are not real user input.** Messages like "Plan accepted. Now implement it fully." are auto-generated by the system when Claude enters plan mode — they are NOT from the user. Do not interpret them as evidence of user dissatisfaction or incomplete work. Focus on Claude's actual output to determine if the task was completed.
12. **Extract suggestions individually** as separate "consider" todos, not as a single "review suggestions" item.
13. **Todos vs insights**: new_todos = tasks to do; insights = things to know. Don't mix.
